...
my %config = (proc => "l00http_blogend_proc",
              desc => "l00http_blogend_desc");
my ($buffer, $lastbuf, $quicktimesave);
$lastbuf = '';
$quicktimesave = 'checked';


sub blogend_make_hdr {
...
sub l00http_blogend_desc {
    my ($main, $ctrl) = @_;      #$ctrl is a hash, see l00httpd.pl for content definition
    # Descriptions to be displayed in the list of modules table
    # at http://localhost:20337/
    "blog: Simple blogger at the end: must be invoked through ls.pl file view";
}
...
sub l00http_blogend_proc {
    my ($main, $ctrl) = @_;      #$ctrl is a hash, see l00httpd.pl for content definition
    my $sock = $ctrl->{'sock'};     # dereference network socket
    my $form = $ctrl->{'FORM'};     # dereference FORM data
    my (@alllines, $line, $lineno, $path, $buforg,  $fname, $pname, $buforgpre);
    ...
    if (defined ($form->{'newtime'})) {
        ...
        $buffer = &blogend_get_msg ($form->{'buffer'}, $stylecurr);
        $form->{'buffer'} = &blogend_make_hdr ($ctrl, $stylecurr, $addtime);
        $form->{'buffer'} .= $buffer;
    }
    ...
    if (defined ($form->{'timesave'})) {
        ...
        $buffer = &blogend_get_msg ($form->{'buffer'}, $stylecurr);
        $form->{'buffer'} = &blogend_make_hdr ($ctrl, $stylecurr, 0);
        $form->{'buffer'} .= $buffer;
        # fake a 'Save' click
        $form->{'save'} = 1;
    }
    ...
    # make header
    $buffer = &blogend_make_hdr ($ctrl, $stylenew, $addtime);
    ...
    if (&l00httpd::l00freadOpen($ctrl, $form->{'path'})) {
        ...
        while ($_ = &l00httpd::l00freadLine($ctrl)) {
            ...
            if (($key) = /^%BLOG:([^%]+)%/) {
                ...
                $key =~ s/ /+/g;
                # /blogend.htm?timesave=&buffer=20140307+135828+key&save=Save&path=C%3A%2Fx%2Fdel.txt
                print $sock "<a href=\"/blogend.htm?timesave=&buffer=20140101+000000+$key&save=Save&path=$form->{'path'}\">$key</a>\n";
                $keys++;
            }
            # trim width
