...
#                    $plain2 .= "<a href=\"/clip.pl?clip=$buffer&update=y\">$name</a> ";
#/mkclip.pl?path=/sdcard/l00httpd/NtMevpSdk.txt&clip=root%0A#end
                    $plain2 .= "<a href=\"/mkclip.pl?path=$path&clip=$buffer$anchor\">$name</a> "; 
                }
                # wiki tags
                if ($needsnewline == 0) {
                    $plain2 .= "$lvl$title$lvl\n";
                } else {
                    $needsnewline = 0;
                    $plain2 .= "\n$lvl$title$lvl\n";
                }
