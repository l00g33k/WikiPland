...
my %config = (proc => "l00http_gpsmap_proc",
              desc => "l00http_gpsmap_desc");
...
#print ",,,($lathtm < $mapbrlat) { $lathtm = $mapbrlat; \n";
    ...
    if ($lonhtm > $mapbrlon) { $lonhtm = $mapbrlon; $notclip = 0; print "1\n";}
    if ($lonhtm < $maptllon) { $lonhtm = $maptllon; $notclip = 0; print "2\n";}
    if ($lathtm > $maptllat) { $lathtm = $maptllat; $notclip = 0; print "3\n";}
    if ($lathtm < $mapbrlat) { $lathtm = $mapbrlat; $notclip = 0; print "4\n";}
...
sub l00http_gpsmap_proc (\%) {
    ...
    print $sock $ctrl->{'httphead'} . $ctrl->{'htmlhead'} . "<title>gpsmap</title>" . $ctrl->{'htmlhead2'};
    ...
    if (open (IN, "<$path")) {
        ...
        ($pixx, $pixy, $notclip) = &ll2xy ($lon, $lat);
#print ",,,left:$pixx"."px; top:$pixy"."px $notclip\n";
        ...
        if (open (WAY, "<$waypts")) {
            ...
            while (<WAY>) {
                ...
                if (/^T +([NS])(\d\d)([0-9.\-]+) +([EW])(\d\d\d)([0-9.\-]+)/) {
                    ...
                    ($pixx, $pixy, $notclip) = &ll2xy ($plon, $plat);
                    #print "$1 $4 $plat $plon ";
        ...
        print $sock "Click map below to move cursor:<br><form action=\"/gpsmap.htm\" method=\"get\">\n";
    ...
    print $sock "<form action=\"/gpsmap.htm\" method=\"get\">\n";
